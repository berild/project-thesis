...
<p style="color:red;background-color:yellow;">This might be wrong! mean of previous $\beta$. Currently being tested</p>
...
$$
thereby yielding the posterior marginal distributions 
$$
...
\end{align}
$$
We can sample from the conditional marginal of $\tau$, but there are no known distribution for the conditional marginal of $\beta$. Therefore we must find posterior marginals of $\beta$ by a Metropolitan-Hastings step, with block update on the model parameters. 

## `r icon:: ii("arrow-right-b")``r icon:: ii("arrow-right-b")`  Method

```{r prior.beta, eval=FALSE}
...
Using the Markov chain Monte Carlo we need to draw samples from the conditional marginal of $\tau$. This is done by the function **r.tau()**.

```{r r.tau, eval=FALSE}
r.tau <- function(data, beta){
  shape = 1.5
  rate = 1/(1/(5*10^(-5)) + t(data$y - data$x%*%beta)%*%(data$y - data$x%*%beta))
...
$$
\alpha = \min\left\{1, \frac{\pi(\vect{y}\given\beta^*,\tau)\pi(\beta^*)q(\beta\given\beta^*)}{\pi(\vect{y}\given\beta,\tau)\pi(\beta)q(\beta^*\given\beta)}\right\}.
$$
...
$$
\alpha = \min\left\{1,\frac{\pi(\vect{y}\given\beta^*)\pi(\beta^*)q(\beta\given\beta^*)}{\pi(\vect{y}\given\beta)\pi(\beta)q(\beta^*\given\beta)}\right\}
$$

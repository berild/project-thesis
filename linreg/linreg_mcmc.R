...
r.tau <- function(y,x,beta,alpha,a = 1,b = 5e-05){
  rgamma(1, shape = a + 0.5, scale = 1/(1/b + t(y- alpha - x%*%beta)%*%(y- alpha - x%*%beta)/2))
}
...
d.beta <- function(y,x,alpha,beta,tau,sigma =0.001){
  one = rep(1,length(y))
  return(-tau/2*(-t(y)%*%(x%*%beta) + 
                   alpha*t(one)%*%(x%*%beta) -
                   t(x%*%beta)%*%y +
                   alpha*t(x%*%beta)%*%one + 
                   t(x%*%beta)%*%(x%*%beta)) - 
...
linreg.mcmc <- function(data,n.samples=100,n.burnin=5,n.thin = 1){
  ...
  for (i in seq(2,n.samples)){
    ...
    alpha.new = r.alpha(data$y,data$x,beta.new,chain$tau[i-1])
    tau.new = r.tau(data$y,data$x,beta.new,alpha.new)
